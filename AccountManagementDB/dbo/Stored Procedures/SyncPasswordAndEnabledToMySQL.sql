CREATE PROCEDURE [dbo].[SyncPasswordAndEnabledToMySQL]
AS
	...
	set @openquery='select * from openquery(PROXY,'''+@select+@where+''')'
	print @openquery
	...
	where chXData<>'' --and chDeptName not like '花蓮%' and ltrim(rtrim(chUserID))<>''
		--and chDeptName like '%法人%' 
		and ltrim(rtrim(chUserID))<>''
		and len(ltrim(rtrim(chUserID)))<10
	...
	begin
		...
		begin
			...
			begin
				...
				set @openquery='update openquery(PROXY,'''+@select+@where+''' ) set password='''+lower(@chXData)+''' '
				print @openquery
			...
			begin
				...
				begin
					...
					set @openquery='update openquery(PROXY,'''+@select+@where+''' ) set enabled=''1'' '
					print @openquery
			...
			begin
				...
				begin
					...
					set @openquery='update openquery(PROXY,'''+@select+@where+''' ) set enabled=''0'' '
					print @openquery
		...
		begin
			...
			insert openquery(PROXY,'select user,password,enabled,Dept,mail_address,fullname,comment from passwd where 1=0 ')
			values
